...
public class ItemFastSword
extends ItemModSword {
    private Random rand = new Random();

    public ItemFastSword(String name, Item.ToolMaterial mat) {
        super(mat, name);
    }

    public boolean hitEntity(ItemStack stack, EntityLivingBase live, EntityLivingBase player) {
        if (this.rand.nextInt(99) < 10) {
            player.addPotionEffect(new PotionEffect(Potion.moveSpeed.id, 60, 4));
        }
    ...
    public void addInformation(ItemStack item, EntityPlayer player, List list, boolean par4) {
        ...
        list.add(ChatFormatting.GOLD + "10% " + I18n.format((String)"items.fastsword", (Object[])new Object[0]));
