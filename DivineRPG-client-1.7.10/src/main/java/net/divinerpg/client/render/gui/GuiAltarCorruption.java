...
extends GuiContainer {
    ...
    public void updateScreen() {
        ...
        if (!ItemStack.areItemStacksEqual((ItemStack)itemstack, (ItemStack)this.itemstack)) {
            this.itemstack = itemstack;
            do {
                this.bookY += (float)(this.rand.nextInt(4) - this.rand.nextInt(4));
        ...
        this.bookZ = flag ? (this.bookZ += 0.2f) : (this.bookZ -= 0.2f);
        if (this.bookZ < 0.0f) {
            this.bookZ = 0.0f;
        }
        if (this.bookZ > 1.0f) {
        ...
        if ((f1 = (this.bookY - this.bookX) * 0.4f) < -(f = 0.2f)) {
            f1 = -f;
        }
        if (f1 > f) {
            f1 = f;
