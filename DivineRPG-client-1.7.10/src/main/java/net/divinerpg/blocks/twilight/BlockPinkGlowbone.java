...
public class BlockPinkGlowbone
extends BlockMod
implements IPlantable {
    private IIcon[] iconArray = new IIcon[2];

    public BlockPinkGlowbone() {
        super(EnumBlockType.PLANT, "pinkGlowbonePlant", true);
        float var3 = 0.375f;
    ...
    protected void checkBlockCoordValid(World world, int i, int j, int k) {
        if (!this.canBlockStay(world, i, j, k)) {
            if (world.getBlockMetadata(i, j, k) == 2) {
                ...
                this.dropBlockAsItem(world, i, j, k, new ItemStack(TwilightItemsCrops.pinkGlowboneSeeds));
            }
    ...
    public Item getItemDropped(int meta, Random rand, int fortune) {
        return TwilightItemsCrops.pinkGlowboneSeeds;
    }
    ...
    public ArrayList<ItemStack> getDrops(World world, int x, int y, int z, int meta, int fortune) {
        ...
        if (meta == 2) {
            drops.add(new ItemStack(TwilightItemsCrops.pinkGlowbone, 1));
