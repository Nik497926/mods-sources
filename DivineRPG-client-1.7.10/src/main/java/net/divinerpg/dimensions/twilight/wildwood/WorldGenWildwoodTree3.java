...
public class WorldGenWildwoodTree3
extends WorldGenerator {
    ...
    public WorldGenWildwoodTree3(boolean check) {
        this.regardless = check;
    }

    public boolean generate(World w, Random rand, int i, int j, int k) {
        int height = rand.nextInt(5) + 1;
        if (!(this.regardless || w.getBlock(i += 3, j, k += 3) == Blocks.air && w.getBlock(i + 3, j, k) == Blocks.air && w.getBlock(i - 3, j, k) == Blocks.air && w.getBlock(i, j, k + 3) == Blocks.air && w.getBlock(i, j, k - 3) == Blocks.air && w.getBlock(i, j + height, k) == Blocks.air && w.getBlock(i, j - 1, k) == TwilightBlocks.wildwoodGrass)) {
            return false;
        }
        if (this.regardless && (w.getBlock(i, j, k) != Blocks.air || w.getBlock(i, j + height, k) != Blocks.air || w.getBlock(i, j - 1, k) != TwilightBlocks.wildwoodGrass)) {
            return false;
        }
        for (int y = 0; y < height; ++y) {
            w.setBlock(i, j + y, k, TwilightBlocks.wildwoodLogs);
            if (rand.nextInt(3) == 0) {
                w.setBlock(i + 1, j + y, k, TwilightBlocks.wildwoodVine, 2, 2);
            }
            if (rand.nextInt(3) == 0) {
                w.setBlock(i - 1, j + y, k, TwilightBlocks.wildwoodVine, 8, 2);
            }
            if (rand.nextInt(3) == 0) {
                w.setBlock(i, j + y, k - 1, TwilightBlocks.wildwoodVine, 1, 2);
            }
            if (rand.nextInt(3) != 0) continue;
            w.setBlock(i, j + y, k + 1, TwilightBlocks.wildwoodVine, 4, 2);
        }
        w.setBlock((i -= 3) + 0, (j += height) + 1, (k -= 3) + 3, TwilightBlocks.wildwoodLeaves);
