...
import net.divinerpg.dimensions.vethea.FloorTexture;
...
public class VetheanChunkBuilder {
    ...
    public VetheaChunk buildChunk(int chunkX, int chunkZ) {
        ...
        if (rand.nextInt(5) == 0) {
            floorTexture.generate(chunk, 8, 16, 8);
        }
        if (rand.nextInt(5) == 0) {
            floorTexture.generate(chunk, 8, 64, 8);
        }
        if (rand.nextInt(5) == 0) {
            floorTexture.generate(chunk, 8, 112, 8);
        }
        if (rand.nextInt(5) == 0) {
            floorTexture.generate(chunk, 8, 160, 8);
        }
        return chunk;
    }
}
