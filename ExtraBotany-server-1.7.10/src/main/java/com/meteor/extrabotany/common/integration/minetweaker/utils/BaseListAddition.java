        ...
        if (!this.recipes.isEmpty()) {
            for (Object recipe : this.recipes) {
                if (recipe != null) {
                    ...
                    LogHelper.error(String.format("Error adding %s Recipe for %s", this.name, this.getRecipeInfo(recipe)), new Object[0]);
                    continue;
                }
                LogHelper.error(String.format("Error adding %s Recipe: null object", this.name), new Object[0]);
            }
        }
    }
