...
// TODO gamerforEA use DivineTeleporter.setBlock(...)
public class DivineTeleporter extends Teleporter
{
			...
			for (int l = -2; l <= 2; ++l)
			{
				for (int i1 = -2; i1 <= 2; ++i1)
				{
					for (int j1 = -1; j1 < 3; ++j1)
					{
						...
						int zz = k + i1 * b1 - l * b0;
						boolean flag = j1 < 0;
						this.setBlock(xx, yy, zz, flag ? this.portal : Blocks.air);
					}
				}
			}

			entity.setLocationAndAngles(i, j, k, entity.rotationYaw, 0);
	...
	public boolean placeInExistingPortal(Entity entity, double x, double y, double z, float rotation)
	{
		...
		if (this.destinationCoordinateCache.containsItem(j1))
		{
			...
			for (l3 = l - short1; l3 <= l + short1; ++l3)
			{
				...
				for (int l1 = i1 - short1; l1 <= i1 + short1; ++l1)
				{
					...
					{
						if (this.worldServerInstance.getBlock(l3, i2, l1) == this.portal)
						{
		...
		if (d3 >= 0)
		{
			...
			if (i4 > -1)
			{
				...
				else if (i4 == Direction.facingToDirection[j2])
				{
					f5 = 1.0F;
					f6 = -1.0F;
				}
				...
				entity.rotationYaw = rotation - j2 * 90 + i4 * 90;
			}
			else
			...
			for (i3 = -1; i3 <= 1; ++i3)
			{
				for (j3 = 1; j3 < 3; ++j3)
				{
					for (k3 = -1; k3 < 3; ++k3)
					{
						...
						j4 = k2 + (j3 - 1) * l2 - i3 * l5;
						flag = k3 < 0;
						this.setBlock(l3, i4, j4, flag ? this.frameBlock : Blocks.air);
					}
				}
			}
		}
		...
		for (i3 = 0; i3 < 4; ++i3)
		{
			for (j3 = 0; j3 < 4; ++j3)
			{
				for (k3 = -1; k3 < 4; ++k3)
				{
					...
					this.setBlock(l3, i4, j4, flag ? this.frameBlock : this.portal, 0, 2);
				}
			}
	...
	private final boolean setBlock(int x, int y, int z, Block block)
	{
		if (!EventConfig.portalGeneration)
			return false;
		if (EventConfig.teleportGenPrivateCheck && EventUtils.isInPrivate(this.worldServerInstance, x, y, z))
			return false;

		if (EventConfig.portalStoneFrame && block == this.frameBlock)
			block = Blocks.stone;

		return this.worldServerInstance.setBlock(x, y, z, block);
	}

	private final boolean setBlock(int x, int y, int z, Block block, int meta, int flags)
	{
		if (!EventConfig.portalGeneration)
			return false;
		if (EventConfig.teleportGenPrivateCheck && EventUtils.isInPrivate(this.worldServerInstance, x, y, z))
		...
		return this.worldServerInstance.setBlock(x, y, z, block, meta, flags);
	}
