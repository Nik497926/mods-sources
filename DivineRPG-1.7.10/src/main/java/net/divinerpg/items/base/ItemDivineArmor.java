...
implements ISpecialArmor {
    ...
    public ItemDivineArmor(EnumArmor armorMaterial, int type, Object[] info, String skin) {
        ...
        for (int i = 0; i < this.armorInfo.length; ++i) {
            ...
            if (strInfo.contains("#")) {
                try {
                    ...
                    strInfo = TokenHelper.replaceToken(strInfo, '#', (int)value);
                }
            ...
            if (i % 2 == 0 && i != 0) {
                this.infoBuilder.append(Util.YELLOW);
            }
            if (i == this.armorInfo.length - 1) {
                this.infoBuilder.append(Util.YELLOW + strInfo);
                continue;
            }
    ...
    public void addInformation(ItemStack item, EntityPlayer player, List list, boolean par4) {
        ...
        for (int i = 0; i < ChatFormats.DIMENSIONS_LIST.length; ++i) {
            ...
            perks = perks + "B " + this.armorInfo[0].toString() + ": ";
        }
        perks = perks + this.infoBuilder.toString();
        for (int c = 0; c < ChatFormats.DIMENSIONS_LIST.length; ++c) {
            perks = perks.replace(Util.YELLOW + ChatFormats.DIMENSIONS_LIST[c], "");
        }
