...
public class EventEnsureVetheaSpawn {
    ...
    public void tick(TickEvent.PlayerTickEvent evt) {
        HashMap map;
        ChunkCoordinates c;
        if (!evt.player.worldObj.isRemote && evt.player.dimension == ConfigurationHelper.vethea && (c = (ChunkCoordinates)(map = Util.getPlayerSpawnChunkMap(evt.player)).get(ConfigurationHelper.vethea)) != null) {
            int x = c.posX;
            int y = c.posY;
            int z = c.posZ;
    ...
    public void place(BlockEvent.PlaceEvent evt) {
        HashMap map;
        ChunkCoordinates c;
        if (evt.player.dimension == ConfigurationHelper.vethea && (c = (ChunkCoordinates)(map = Util.getPlayerSpawnChunkMap(evt.player)).get(ConfigurationHelper.vethea)) != null) {
            int x = c.posX;
            int y = c.posY;
            int z = c.posZ;
