...
// TODO gamerforEA use TeleporterVethea.setBlock(...)
public class TeleporterVethea extends Teleporter
{
	...
	private void makePortalAt(World world, int x, int y, int z)
	{
		...
		for (int i = 0; i < 5; i++)
		{
			for (int j = 0; j < 5; j++)
			{
				for (int k = 0; k < 6; k++)
				{
					setBlock(world, x + i - 3, y + j + 1, z + k - 4, Blocks.air);
				}
			}
		}

		setBlock(world, x + 1, y, z, TwilightBlocks.mortumBlock);
		setBlock(world, x, y, z, TwilightBlocks.mortumBlock);
		setBlock(world, x + 2, y, z, TwilightBlocks.mortumBlock);
		setBlock(world, x + 1, y + 1, z - 1, TwilightBlocks.mortumBlock);
		setBlock(world, x + 1, y + 1, z + 1, TwilightBlocks.mortumBlock);
		setBlock(world, x + 1, y + 2, z - 2, TwilightBlocks.mortumBlock);
		setBlock(world, x + 1, y + 2, z + 2, TwilightBlocks.mortumBlock);
		setBlock(world, x + 1, y + 3, z + 1, TwilightBlocks.mortumBlock);
		setBlock(world, x + 1, y + 3, z - 1, TwilightBlocks.mortumBlock);
		setBlock(world, x + 1, y + 4, z, TwilightBlocks.mortumBlock);
		setBlock(world, x + 1, y + 1, z, VetheaBlocks.vetheaPortal);
		setBlock(world, x + 1, y + 2, z + 1, VetheaBlocks.vetheaPortal);
		setBlock(world, x + 1, y + 2, z, VetheaBlocks.vetheaPortal);
		setBlock(world, x + 1, y + 2, z - 1, VetheaBlocks.vetheaPortal);
		setBlock(world, x + 1, y + 3, z, VetheaBlocks.vetheaPortal);
	}
	...
	private static final boolean setBlock(World world, int x, int y, int z, Block block)
	{
		...
		if (EventConfig.portalStoneFrame && block == TwilightBlocks.mortumBlock)
			block = Blocks.stone;
