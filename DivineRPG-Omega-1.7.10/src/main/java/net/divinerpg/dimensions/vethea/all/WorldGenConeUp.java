    ...
    void placeBlockCircle(World world, Random rand, int x, int y, int z, int radius) {
        ...
        while (i < (float)radius) {
            ...
            while ((double)j < Math.PI * 2 * (double)i) {
                ...
                if (rand.nextInt(6) == 0) {
                    world.setBlock((int)Math.floor((double)x + Math.sin(j) * (double)i) + rand.nextInt(2) - rand.nextInt(2), y + 1, (int)Math.floor((double)z + Math.cos(j) * (double)i) + rand.nextInt(2) - rand.nextInt(2), this.block);
                }
                if (rand.nextInt(6) == 0) {
                    world.setBlock((int)Math.floor((double)x + Math.sin(j) * (double)i) + rand.nextInt(2) - rand.nextInt(2), y - 1, (int)Math.floor((double)z + Math.cos(j) * (double)i) + rand.nextInt(2) - rand.nextInt(2), this.block);
                }
