...
extends ItemBow {
    ...
    public void onPlayerStoppedUsing(ItemStack stack, World world, EntityPlayer player, int itemUseCount) {
        ...
        boolean infiniteAmmo = !this.needArrow || player.capabilities.isCreativeMode || EnchantmentHelper.getEnchantmentLevel((int)Enchantment.infinity.effectId, (ItemStack)stack) > 0;
        boolean bl = infiniteAmmo;
    ...
    public void addInformation(ItemStack item, EntityPlayer player, List list, boolean par4) {
        int powerLevel = EnchantmentHelper.getEnchantmentLevel((int)Enchantment.power.effectId, (ItemStack)item);
        double damMin = (double)this.damageMin + (double)powerLevel * 0.5 + 0.5 + 1.0;
        double damMax = (double)this.damageMax + (double)powerLevel * 0.5 + 0.5 + 1.0;
        String a = Double.toString(damMin);
        String b = Double.toString(damMax);
        if (powerLevel != 0) {
            if (a.contains(".0") || b.contains(".0")) {
                a = a.replace(".0", "");
                b = b.replace(".0", "");
                list.add(TooltipLocalizer.bowDam(a + Util.WHITE + " - " + Util.GREEN + ChatFormatting.BOLD + b));
            } else {
                list.add(TooltipLocalizer.bowDam("" + damMin + Util.WHITE + " - " + Util.GREEN + ChatFormatting.BOLD + damMax));
            }
        } else {
            list.add(TooltipLocalizer.bowDam("" + this.damageMin + Util.WHITE + " - " + Util.GREEN + ChatFormatting.BOLD + this.damageMax));
        }
        double speed = 72000.0 / (double)this.getMaxItemUseDuration(item);
        if (speed > 1.0) {
            list.add(Util.GOLD + (int)speed + StatCollector.translateToLocal((String)"item.bowfaster"));
        }
        if (speed < 1.0) {
            list.add(Util.GOLD + 1 / (int)speed + StatCollector.translateToLocal((String)"item.bowslower"));
        }
        if (this.unbreakable) {
        ...
        if (this.arrowTex == "hunterArrow") {
            list.add(TooltipLocalizer.poisonShots());
        }
        if (this.arrowTex == "witherArrow") {
            list.add(TooltipLocalizer.witherShots());
        }
        if (this.arrowTex == "fireArrow") {
            list.add(TooltipLocalizer.burn(5));
        }
    ...
    public ItemStack onItemRightClick(ItemStack stack, World world, EntityPlayer player) {
        ...
        boolean infiniteAmmo = !this.needArrow || player.capabilities.isCreativeMode || EnchantmentHelper.getEnchantmentLevel((int)Enchantment.infinity.effectId, (ItemStack)stack) > 0 && player.inventory.hasItem(this.arrow);
        boolean bl = infiniteAmmo;
        if (infiniteAmmo || player.inventory.hasItem(this.arrow)) {
