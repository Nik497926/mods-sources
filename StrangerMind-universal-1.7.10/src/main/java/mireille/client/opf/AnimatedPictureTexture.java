...
public class AnimatedPictureTexture extends PictureTexture
{
    ...
    public AnimatedPictureTexture(final ProcessedImageData image) {
    ...
    public int getTextureID() {
        final long time = (this.duration > 0L) ? (System.currentTimeMillis() % this.duration) : 0L;
        int index = 0;
    ...
    private int uploadFrame(final int index) {
        final int id = this.imageData.uploadFrame(index);
