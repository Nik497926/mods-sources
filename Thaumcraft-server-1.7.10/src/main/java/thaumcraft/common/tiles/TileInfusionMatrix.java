	...
	public void craftingStart(EntityPlayer player)
	{
		...
		else
		{
			...
			if (te instanceof TilePedestal)
			{
				...
				ItemStack stackInSlot = pedestal.getStackInSlot(0);
			...
			if (this.recipeInput != null)
			{
				...
				if (components.size() != 0)
				{
					...
					if (recipe != null)
					{
						Object output = recipe.getRecipeOutput(this.recipeInput);
	...
	public void craftingFinish(Object out, String label)
	{
		...
		if (te instanceof TilePedestal)
		{
			...
			else if (out instanceof Enchantment)
			{
				...
				// TODO gamerforEA code start
				if (this.recipeInput == null || ItemStack.areItemStacksEqual(this.recipeInput, temp))
				// TODO gamerforEA code end
				{
	...
	private void getSurroundings()
	{
		...
		try
		{
			...
			for (ChunkCoordinates cc : this.pedestals)
			{
				...
				{
					this.symmetry += 2;
					ItemStack stackInSlot = ((TilePedestal) te).getStackInSlot(0);
		...
		for (String fxk : this.sourceFX.keySet().toArray(new String[0]))
		{
			...
			else
			{
				...
				else
				{
					...
					{
						ItemStack stackInSlot = ((TilePedestal) tile).getStackInSlot(0);
