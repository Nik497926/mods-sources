...
import static com.gamerforea.thaumcraft.ChatUtils.color;
...
public class BlockMetalDevice extends BlockContainer
{
	...
	public void breakBlock(World world, int x, int y, int z, Block block, int meta)
	{
		...
		if (te instanceof TileCrucible)
			...
			world.spawnEntityInWorld(new EntityItem(world, x + 0.5F, y + 0.5F, z + 0.5F, new ItemStack(ConfigItems.itemResource, 1, 13)));
		else if (te instanceof TileArcaneLamp)
	...
	public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float par7, float par8, float par9)
	{
		...
		if (metadata == 1 && !world.isRemote && !player.isSneaking() && player.getHeldItem() == null)
		{
			...
			if (te instanceof TileAlembic)
			{
				...
				{
					if (tile.amount < tile.maxAmount * 0.4D)
						msgKey = "tile.alembic.msg.2";
					else if (tile.amount < tile.maxAmount * 0.8D)
						msgKey = "tile.alembic.msg.3";
					else if (tile.amount < tile.maxAmount)
						msgKey = "tile.alembic.msg.4";
					else if (tile.amount == tile.maxAmount)
						msgKey = "tile.alembic.msg.5";
				}
				else
					msgKey = "tile.alembic.msg.1";

				if (msgKey != null)
					player.addChatComponentMessage(color(translate(msgKey), EnumChatFormatting.DARK_AQUA));
				// TODO gamerforEA code end
