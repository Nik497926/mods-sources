	...
	public static boolean replaceArcaneFurnace(World world, int x, int y, int z)
	{
		...
		for (int yOffset = 0; yOffset < 3; ++yOffset)
		{
			...
			for (int zOffset = 0; zOffset < 3; ++zOffset)
			{
				for (int xOffset = 0; xOffset < 3; ++xOffset)
				{
					...
					{
						int arcaneFurnaceMeta = step;

						Block block = world.getBlock(xx, yy, zz);
						if (block == Blocks.lava || block == Blocks.flowing_lava)
							arcaneFurnaceMeta = 0;
						else if (block == Blocks.iron_bars)
							arcaneFurnaceMeta = 10;

						world.setBlock(xx, yy, zz, ConfigBlocks.blockArcaneFurnace, arcaneFurnaceMeta, 0);
						world.addBlockEvent(xx, yy, zz, ConfigBlocks.blockArcaneFurnace, 1, 4);
					}
		...
		for (int yOffset = 0; yOffset < 3; ++yOffset)
		{
			for (int zOffset = 0; zOffset < 3; ++zOffset)
			{
				for (int xOffset = 0; xOffset < 3; ++xOffset)
				{
					world.markBlockForUpdate(x + xOffset, y + yOffset, z + zOffset);
				}
			}
		}
