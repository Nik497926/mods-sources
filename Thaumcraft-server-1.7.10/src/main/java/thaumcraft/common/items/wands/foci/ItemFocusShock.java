	...
	public void doLightningBolt(ItemStack stack, EntityPlayer p, int count)
	{
		...
		else
		{
			...
			else
			{
				...
				// TODO gamerforEA code replace, old code: if (pointedEntity instanceof EntityLivingBase && (!(pointedEntity instanceof EntityPlayer) || MinecraftServer.getServer().isPVPEnabled()))
					...
					if (cl > 0)
					{
						...
						while (cl > 0)
						{
							...
							if (closest != null)
							{
								...
								if (!EventUtils.cantDamage(p, closest))
