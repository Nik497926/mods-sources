...
public class InventoryUtils
{
	...
	public static boolean areItemStacksEqualForCrafting(ItemStack stack0, ItemStack stack1, boolean useOre, boolean ignoreDamage, boolean ignoreNBT)
	{
		...
		else if (stack0.getItem() == null || stack1.getItem() == null)
			return false;
		...
		else
		{
			...
			if (t2 && ignoreDamage && (stack0.getItemDamage() == OreDictionary.WILDCARD_VALUE || stack1.getItemDamage() == OreDictionary.WILDCARD_VALUE))
				t2 = false;
			...
			for (int i = 0; i < inventory.getSizeInventory(); ++i)
			{
				if ((!(tileEntity instanceof TileResearchTable) || md != 15 || i != 9) && (!(tileEntity instanceof TileArcaneWorkbench) || i != 9))
				{
					...
					if (item != null && item.stackSize > 0)
					{
						...
						EntityItem entityItem = new EntityItem(world, (double) ((float) x + rx), (double) ((float) y + ry), (double) ((float) z + rz), item.copy());
						item.stackSize = 0;
						// TODO gamerforEA code end
	...
	public static ItemStack cycleItemStack(Object input)
	{
		...
		{
			it = (ItemStack) input;
			if (it.getItemDamage() == OreDictionary.WILDCARD_VALUE && it.getItem().getHasSubtypes())
			{
			...
			else if (it.getItemDamage() == OreDictionary.WILDCARD_VALUE && it.isItemStackDamageable())
			{
