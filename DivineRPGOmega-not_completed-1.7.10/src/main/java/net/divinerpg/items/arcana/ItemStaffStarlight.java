...
extends ItemModRanged {
    ...
    public ItemStack onItemRightClick(ItemStack stack, World world, EntityPlayer player) {
        ...
        float pitchAngle = -MathHelper.cos((float)(-rotationPitch * 0.01745329f));
        float xVec = yawAngleSin * pitchAngle;
        double multiplyer = 30.0;
        float yVec = MathHelper.sin((float)(-rotationPitch * 0.01745329f));
        Vec3 worldVector2 = worldVector.addVector((double)xVec * 30.0, (double)yVec * multiplyer, (double)(zVec = yawAngleCos * pitchAngle) * multiplyer);
    ...
    protected void addAdditionalInformation(ItemStack stack, EntityPlayer player, List list, boolean par4) {
        ...
        list.add(stars ? Util.GOLD + StatCollector.translateToLocal((String)"item.starlight1") : Util.GOLD + StatCollector.translateToLocal((String)"item.starlight2"));
