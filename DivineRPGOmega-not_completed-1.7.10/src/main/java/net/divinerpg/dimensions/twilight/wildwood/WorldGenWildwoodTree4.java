...
public class WorldGenWildwoodTree4
extends WorldGenerator {
    public boolean generate(World w, Random rand, int i, int j, int k) {
        int height = rand.nextInt(5) + 1;
        if (w.getBlock(i += 2, j, k += 2) != Blocks.air || w.getBlock(i + 3, j, k) != Blocks.air || w.getBlock(i - 3, j, k) != Blocks.air || w.getBlock(i, j, k + 3) != Blocks.air || w.getBlock(i, j, k - 3) != Blocks.air || w.getBlock(i, j + height, k) != Blocks.air || w.getBlock(i, j - 1, k) != TwilightBlocks.wildwoodGrass) {
            return false;
        }
        ...
        w.setBlock((i -= 2) + 0, (j += height) + 1, (k -= 2) + 2, TwilightBlocks.wildwoodLeaves);
